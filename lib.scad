module bearing_holder(l, h, r1, r2, holder_depth, holder_thick=2) {
	union(){
		difference(){
			...
			cylinder (h = l, r=r2, center = true, $fn=100);
			translate([0,-r2,0]) cylinder (h = l, r=r2, center = true, $fn=100);
		}
...
module single_bearing_frame(r1, r2, h, l, d, holder_depth, holder_thick=2, bar_offset=false){
	difference(){
		...
		translate([0,-(l/2+(d+l)/4+holder_thick/2),h/4]) cube([r1*2,(d+l)/2-holder_thick,h/2], center=true);
		translate([0,(l/2+(d+l)/4+holder_thick/2),h/4]) cube([r1*2,(d+l)/2-holder_thick,h/2], center=true);
	}
}
...
module double_bearing_frame(r1, r2, h, l, d, holder_depth, holder_thick=2, bar_offset=false){
	difference(){
		...
		translate([0,0,h/4]) cube([r1*2,d-4*holder_depth,h/2], center=true);
	}
}
